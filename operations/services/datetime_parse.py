...
def format_datetime_display(dt: datetime) -> str:
    tz = _business_tz()
    local = timezone.localtime(dt, timezone=tz) if timezone.is_aware(dt) else dt.replace(tzinfo=tz)
...
def parse_datetime_text(text: str, default: datetime | None = None) -> datetime:
    ...
    Wall times are interpreted in the active business timezone.
    """
    text = (text or '').strip()
    if not text:
    ...
    tz = _business_tz()
    base = default or timezone.localtime(timezone.now(), timezone=tz)
    if timezone.is_aware(base):
        base = timezone.localtime(base, timezone=tz)
