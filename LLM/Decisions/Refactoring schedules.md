# Decision: Package scheduling domain documentation

**Status:** accepted  
**Live spec:** `LLM/domains/scheduling.md` (shim) + `LLM/domains/scheduling/` (`index`, `booking`, `checkin`, `capacity`, `dashboard`, `pricing`, `calendar_email`)  
**What we took:** Domain-package pattern for docs; selective context loading; index + topic files. Expanded from the original five files to seven (`dashboard.md`, `pricing.md` added). Content aligned with operations via the companion decision on the scheduling markdown refactor.  
**What we left:** Five-file-only split; leaving a god `scheduling.md` as live spec.  
**Why:** Same reason as code packages — small, auditable units and lower LLM token burn.  
**Note:** Original filename in proposals was `Refactoring schedlules.md` (typo).
...
Does breaking `scheduling.md` into this modular folder structure match how you want to streamline the domain documentation?