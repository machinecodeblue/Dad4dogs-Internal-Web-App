# Decision: Dog model refactor

**Status:** accepted  
**Live spec:** `operations/models/customers/dogs/` (`base`, `querysets`, `pipeline`, `vaccinations`, `feed`); `customers.md`  
**What we took:** Split monolith `dogs.py` into package with mixins; stable `ClientProfile` / `ClientProfileQuerySet` re-exports.  
**What we left:** No schema change.  
**Why:** File was ~338 lines / 12 KB — over PHILOSOPHY small-file threshold.

---

The `ClientProfile` model in `operations/models/customers/dogs.py` has become a large monolithic file mixing several distinct responsibilities:

1. **QuerySet & Aggregation Logic** (`ClientProfileQuerySet` for visibility, vaccination expiry annotations, counts)


2. **Core Model & Direct DB Fields** (`ClientProfile`, constraints, basic properties)


3. **Pipeline State & Readiness Gates** (`standard_stay_blockers`, `evaluation_stay_blockers`, `can_schedule_meet_greet`, `advance_pipeline`)


4. **Vaccination Computation & Expiry Helpers** (`has_current_vaccination`, `current_vaccination_expires_at`, `vaccination_status`)


5. **Customer Feed Credentials & URL Generation** (`ensure_feed_credentials`, `feed_url`, `regenerate_feed_secret`)



Applying the package refactoring pattern previously used on `scheduling/`, convert `operations/models/customers/dogs.py` into a focused directory package: `operations/models/customers/dogs/`.

---

### Target Package Structure (`operations/models/customers/dogs/`)

```
operations/models/customers/dogs/
├── __init__.py      # Canonical re-export of ClientProfile and ClientProfileQuerySet
├── base.py          # Core ClientProfile model, fields, constraints, string representation
├── querysets.py     # ClientProfileQuerySet (vaccination annotations, visibility filters, aggregates)
├── pipeline.py      # PipelineStage enum, stay blockers, readiness checks, stage advancement
├── vaccinations.py  # Vaccination status resolution, expiry properties
└── feed.py          # Customer feed credentials, slug synchronization, URL generation
...
#### 1. `operations/models/customers/dogs/querysets.py`
...
#### 2. `operations/models/customers/dogs/pipeline.py`
...
#### 3. `operations/models/customers/dogs/vaccinations.py`
...
#### 4. `operations/models/customers/dogs/feed.py`
...
#### 5. `operations/models/customers/dogs/base.py`
...
#### 6. `operations/models/customers/dogs/__init__.py`
...
### Verification & Migration Check

1. Delete `operations/models/customers/dogs.py` after creating the `operations/models/customers/dogs/` package directory.


2. Run Django checks to confirm the ORM and URL paths remain unchanged:
