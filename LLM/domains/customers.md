...
| **New Client & Dog** | `/clients/intake/` | One POST: owner + first dog + vet + optional Meet & Greet (`IntakeWizardForm`). Atomic. M&G uses catalog slug **`meet_greet`** ($0, `capacity_exempt`) — not Short Visit. End blank → **+15 minutes**. Skips standard-stay Approved/vax/COI gate; does not consume boarding capacity. Pipeline $\rightarrow$ Meet & Greet if times set, else Inquiry. |
